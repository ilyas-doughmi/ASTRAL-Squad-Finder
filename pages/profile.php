...
<body class="text-white font-body min-h-screen flex flex-col selection:bg-nexusGreen selection:text-black">
        ...
        <div class="grid lg:grid-cols-12 gap-12">
            ...
            <div class="lg:col-span-8 space-y-8">
                ...
                <div>
                    ...
                    <div id="friend_container" class="grid grid-cols-1 md:grid-cols-2 gap-4">
    ...
    <script>
        ...
        function getUserFriends() {
            ...
            let form = new FormData();
            form.append("id", <?= $_SESSION["id"] ?>);
            fetch("../Includes/friend_request/getUserFriends.php", {
                    method: "POST",
                    body: form
                })
                .then(res => res.json())
                .then(data => {
                    data.forEach(function(e) {
                        const card = `
                            ...
                            <div class="relative">
                                <div class="w-12 h-12 rounded-lg bg-gray-800 overflow-hidden">
                                    <img src="${e.profile_img}" class="w-full h-full object-cover">
                                </div>
                                <div class="absolute -bottom-1 -right-1 w-3 h-3 bg-nexusGreen rounded-full border-2 border-[#0f0f0f] animate-pulse"></div>
                            </div>

                            <div>
                                <h4 class="font-heading font-bold text-white group-hover:text-nexusGreen transition-colors">${e.user_name}</h4>
                        ...
                        friend_container.insertAdjacentHTML("afterbegin",card);
